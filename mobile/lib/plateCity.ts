/**
 * Türk plaka il kodu → il adı (01-81). Tamirci dizinini kullanıcının şehrine
 * göre süzmek için aracın plakasından il çıkarır.
 */
...
/** "34 ABC 123" → "İstanbul". Tanınmazsa null. */
export function cityFromPlate(plate: string | null | undefined): string | null {
  if (!plate) return null;
  const m = plate.trim().match(/^(\d{2})/);
  if (!m) return null;
  return PROVINCES[m[1]] ?? null;
}

export default cityFromPlate;
